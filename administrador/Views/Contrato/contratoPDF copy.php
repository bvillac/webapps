...
<body>
	<table class="tbl-hader">
		<tbody>
			<tr>
				<td class="wd33">
					<img src="<?= media() ?>/logo/logo-miller.jpg" alt="Logo">
				</td>
				...
				<td class="text-right wd33">
					<p><h4>DETALLE ORDEN DE COMPRA</h4><br>
						No. Compra <strong><?= $cabCompra['com_numero_orden'] ?></strong><br>
						Fecha: <?= $cabCompra['com_fecha'] ?>  <br>						
					</p>
				</td> 
			</tr>
		</tbody>
	...
	<table class="tbl-detalle">
		...
		<tbody>
			<?php 
				...
				foreach ($detCompra as $producto) {
					$importe = $producto['com_cantidad'] * $producto['com_precio'];
					$subtotal = $subtotal + $importe;
			 ?>
			<tr>
			    <td><?= $producto['item_id'] ?></td>
				<td><?= $producto['Nombre'] ?></td>
				<td class="text-right"><?= SMONEY.' '.formatMoney($producto['com_precio'],2) ?></td>
				<td class="text-center"><?= $producto['com_cantidad'] ?></td>
		...
		<tfoot>
			<tr>
				...
				<td class="text-right"><?= SMONEY.' '.formatMoney($cabCompra['com_valor_bruto'],2) ?></td>
			</tr>
			<tr>
				<td colspan="4" class="text-right">Base 0%:</td>
				<td class="text-right"><?= SMONEY.' '.formatMoney($cabCompra['com_biva0'],2); ?></td>
			</tr>
			<tr>
				<td colspan="4" class="text-right">Base Impuesto %:</td>
				<td class="text-right"><?= SMONEY.' '.formatMoney($cabCompra['com_biva12'],2); ?></td>
			</tr>
			<tr>
				<td colspan="4" class="text-right">Iva 12%:</td>
				<td class="text-right"><?= SMONEY.' '.formatMoney($cabCompra['com_valor_iva'],2); ?></td>
			</tr>
			<tr>
				<td colspan="4" class="text-right">Total:</td>
				<td class="text-right"><?= SMONEY.' '.formatMoney($cabCompra['com_valor_neto'],2); ?></td>
			</tr>
